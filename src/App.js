...
function App() {
  ...
  const [memoriesBtn, setMemoriesBtn] = useState(false);
  const [showPasswordPrompt, setShowPasswordPrompt] = useState(false);
  const [passwordInput, setPasswordInput] = useState('');
  const correctPassword = "cukoo";
  ...
  const [user, setUser] = useState({
    ...
    driveOption: null,
    formAnswers: null,
    passwordsAttempted: null
  });

  const handlePasswordSubmit = () => {
    const updatedPass = [...pass, passwordInput];
    setPass(updatedPass);

    if (passwordInput === correctPassword) {
      setUser(prevState => ({
        ...prevState,
        passwordsAttempted: updatedPass.join(", ")
      }));
  ...
  const sendEmail = () => {
    ...
    emailjs.send('service_uh2pczx', 'template_4r2i0uv', user, 'rQ0nyQ4UBr8rSIUrE')
      .then((response) => {
        ...
        setPhase('finish');
        localStorage.setItem("BirthdayCelebrationDone", "true");
        setCanPlay(false);
      }, (err) => {
  ...
  const handleResetCelebration = () => {
    localStorage.removeItem("BirthdayCelebrationDone");
 ...
 Aaj ka din tumhara hai, aur main bas itna kehna chahta hoon — tum meri life ka sabse pyaara hissa ho. 😊 Tumhari smile sabse khaas hai, aur tumhara saath hi meri khushi hai. Chalo, aaj ka din milke enjoy karte hain. 🎂❤️
  ...
  useEffect(() => {
    ...
    const hasPlayed = localStorage.getItem("BirthdayCelebrationDone");
    if (hasPlayed === "true") {
      setCanPlay(false);
      setPhase('finish');
    } else {
      ...
      const updateCountdown = () => {
        ...
        let birthday = new Date(`${birthdate}, ${currentYear} 21:48:00`);
    ...
    <div className="App">
      ...
      {!showPasswordPrompt ?
        <>
            ...
            <>
              {showCake ? (
                <div className="card celebration-card">
                  ...
                  {memoriesBtn ?
                    <button onClick={setMemo}>Chalo apni ab tak ki memories dekhte hai ➡️</button> : null}
                </div>
              ) : (
          ...
          {phase === 'finish' && (
            <div className="card celebration-card">
              <TypewriterText text={"ALL DONE, CALL ME NOW 🥰"} lineDelay={1200} charDelay={100} />
              <button onClick={handleResetCelebration}>Reset</button>
            </div>
          )}
        ...
        <div className="password-prompt">
          <h2>Enter secret password to start the surprise 🎉</h2>
          <p>Hint: It's a nickname I call you 😘</p>
          <input
            type="password"
            value={passwordInput}
